...
for line in input_xml:
    ...
    if current_bytes < MAX_BYTES:
        ...
        os.system('7z a split_xml_' + output_count + '_%d.xml.7z' % file_count + ' split_xml_' + output_count + '_%d.xml' % file_count)
	os.system('aws s3 cp split_xml_' + output_count + '_%d.xml.7z' % file_count + ' s3://klabwiki/splits/')
        current_bytes = 0
        file_count += 1
        output_xml = open('split_xml_' + output_count + '_%d.xml' % file_count, 'w')
